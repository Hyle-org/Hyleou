...
watchEffect(async () => {
    if (!transactionStore.value.data[tx_hash.value]) {
        const tx = await transactionStore.value.load(tx_hash.value);
        if (!blockStore.value.data[tx.block_hash]) {
            blockStore.value.load(tx.block_hash);
        }
    }
});

const data = computed(() => transactionStore.value.data?.[tx_hash.value]);
const block = computed(() => blockStore.value.data?.[transactionStore.value.data?.[tx_hash.value]?.block_hash]);
            ...
            <div class="bg-white/80 backdrop-blur-sm rounded-2xl shadow-sm p-6 border border-white/20 mb-4">
                <div class="space-y-4">
                    ...
                    <div class="grid grid-cols-[200px_1fr] gap-4 py-3 border-b border-secondary/5">
                        ...
                        <span class="text-secondary">{{ block?.timestamp ? formatTimestamp(block.timestamp) : "?" }}</span>
                    </div>
                </div>
            </div>
