...
export const loadTransactionData = async (txHash: string) => {
    ...
    transactionData[txHash] = {
        ...
        status: data.result.tx_result.code === 0 ? (settled?.settled ? (settled.success ? "success" : "failure") : "sequenced") : "failure",
        type: data.result.tx_result.events.filter((x: any) => x.type === "message")[0]?.attributes.filter((x: any) => x.key === "action")[0]
            .value,
    ...
    if (transactionData[txHash].type === "/hyle.zktx.v1.MsgRegisterContract") {
        transactionData[txHash].contracts = [(parsed as MsgRegisterContract).contractName];
        // There's no need to wait for registration events
        if (transactionData[txHash].status === "sequenced") transactionData[txHash].status = "success";
    } else if (transactionData[txHash].type === "/hyle.zktx.v1.MsgPublishPayloads") {
        transactionData[txHash].contracts = (parsed as MsgPublishPayloads).payloads.map((x) => x.contractName);
    } else {
...
export const loadContractTxs = async (network: string, contract_name: string) => {
