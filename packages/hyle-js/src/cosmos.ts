import type { AccountData, DirectSecp256k1HdWallet, Registry } from "@cosmjs/proto-signing";
...
import { getNetworkApiUrl, network } from "./network.ts";
...
export async function sendExecuteTX(changes: { contractName: string; proof: string }[]) {
    const msgAny = {
...
export async function checkContractExists(contractName: string) {
    const checkExists = await fetch(`${getNetworkApiUrl(network.value)}/hyle/zktx/v1/contract/${contractName}`);
    try {
